...
def vif_reduction_alternate(df,limit=2.5):
    def compute_vif_vector(considered_features):
        X = df[considered_features]
        corr_matrix = X.corr().values
        inv_corr_matrix = np.linalg.inv(corr_matrix)
        vif_values = np.diag(inv_corr_matrix)
        vif = pd.DataFrame({'Variable': considered_features,'VIF': vif_values})
        return vif 

    matrix = df.values
    Q,R = np.linalg.qr(matrix)
    collinear_columns = np.where(np.isclose(np.abs(np.diagonal(R)),0))[0]
    VIF_inf_cols = [df.columns[col_index] for col_index in collinear_columns]

    drop_col = VIF_inf_cols
    process_itr = 1
    features_list = df.columns
    while process_itr > 0:
        print("Iteration : {0}".format(process_itr))
        if len(drop_col) == 0:
            pass
        else:
            features_list = list(set(features_list) - set(drop_col))
        vif_df = compute_vif_vector(features_list).sort_values('VIF',ascending=False)
        top_row = vif_df.iloc[0].tolist()
        print("top row: {0}".format(top_row))
        if top_row[1] > limit:
            drop_col.append(top_row[0])
            process_itr+=1
        else:
            print("VIF process done")
            process_itr = 0
    return drop_col 
...
def backward_selection(df,dv,regression=True,alpha=.05):
    ...
    if regression:
        while flag==0:
            ...
            if (pvalues[drop_index]> alpha or np.isnan(pvalues[drop_index])):
